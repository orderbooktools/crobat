...
def convert_events_to_listdict(events_array):
    listofdict=[]
    dictkeys = ['time', 'event_type', 'price', 'size', 'position', 'best_price', 'spread']
    for event in events_array:
        temp_dict = dict(zip(dictkeys, event))
        listofdict.append(temp_dict)
    print(listofdict[:5])
    return listofdict
...
def pd_excel_save(title, hist_obj_dict):
    print(title)
    print(len(hist_obj_dict))
    hist_obj_df = pd.DataFrame(hist_obj_dict) 
    print(hist_obj_df)
    hist_obj_df = hist_obj_df[1:]
    print(hist_obj_df)
    hist_obj_df = hist_obj_df.sort_values(by='time',ascending=True)
